...
const BeerCard = ({ beer, onEdit, onDelete }) => {
  return (
    <div className="beer-card">
      <h3 className="beer-name">{beer.nombre}</h3>
      <p className="beer-type">{beer.tipo} - {beer.alcohol}% Alc.</p>
      ...
      <div className="beer-actions">
        <button className="btn btn-secondary" onClick={() => onEdit(beer)}>
          Ver Información
        </button>
        <div>
          ...
          <button className="btn btn-danger" onClick={() => onDelete(beer.id)}>
            Eliminar
          </button>
        </div>
      </div>
